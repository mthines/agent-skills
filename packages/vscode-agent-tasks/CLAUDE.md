...
- **Session ↔ artifact correlation** — `findLinkedArtifacts(worktreePath, gitBranch, configuredDirs)` in `lib/session-artifact-correlator.ts` joins a session to its `<worktree>/<dir>/<branchName>/` artifact files. The bucket worktree (longest match for `session.cwd`) is the correlation root, paired with `session.gitBranch`. When any artifact exists, the `SessionItem` becomes collapsible (`contextValue = claudeSessionWithArtifacts`), the row-click `command` is dropped (so single-click expands cleanly), and a `$(play)` inline action exposes Resume. `LinkedArtifactItem` children open via `agentTasks.openMarkdown`. The correlation map is rebuilt on every `buildRootItems` and on every `artifactWatcher.onArtifactChanged` event so chevrons appear/disappear live. Known/recognised artifact slots: `taskPath`, `planPath`, `walkthroughPath`, `diagnosePaths`. Unknown `.md` files (those that are not a known artifact, not a `plan.v*.md` snapshot, and not a `diagnose-*.md` report) are collected in `otherMarkdownPaths` and rendered at the bottom of the artifact child list with the `markdown` icon and a label derived from the filename (extension stripped). Excludes: known artifact filenames (`task.md`, `plan.md`, `walkthrough.md`), versioned plan snapshots (`plan.v*.md`), and diagnose reports (`diagnose-*.md`).
