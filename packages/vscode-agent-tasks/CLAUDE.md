...
| `agentTasks.openOtherMarkdownFile` | Open an "other markdown file" row from either the Agent Tasks or Sessions tree. Context-menu target for `viewItem == otherMarkdownFile` in both `agentTasksExplorer` and `agentSessionsExplorer`. Receives the tree item as its first argument and delegates to `agentTasks.openMarkdown` using the item's `filePath` field. Hidden from command palette. |
