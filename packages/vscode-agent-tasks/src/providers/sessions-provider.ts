 ...
 * (`task.md`, `plan.md`, `walkthrough.md`, or an unknown "other" markdown
 * file) generated by an autonomous-workflow run on the same
 * `(worktree, gitBranch)` pair.
 *
 * Known artifact rows (plan / task / walkthrough / diagnose) open via
 * `agentTasks.openMarkdown` — rendered markdown preview on click, gated by
 * the `agentTasks.openMarkdownInPreview` setting.
 *
 * Unknown / "other" markdown rows (`contextValueOverride = 'otherMarkdownFile'`)
 * open via `agentTasks.openOtherMarkdownFile` — always the editable editor tab,
 * regardless of the preview setting.
 *
 * Icons mirror the Agent Tasks panel so the visual association is consistent.
 */
export class LinkedArtifactItem extends vscode.TreeItem {
  constructor(
    ...
    if (contextValueOverride === 'otherMarkdownFile') {
      this.command = {
