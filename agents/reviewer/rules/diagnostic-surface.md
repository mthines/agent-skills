...
| 5.6   | Post as pending review            | [reviewer.md § Step 5.6](../reviewer.md)                                                   | `event` omitted; `body == ""`; Conventional-Comments prefix on each comment; state verified `PENDING`     |
...
| 5.6   | **Mechanical** checks: `body == ""`; no `event` key; comment body starts with one of 5 Conventional-Comments prefixes; no `gh pr comment`; no `POST /issues/{n}/comments`; verify state is `PENDING` after posting | "Pending isn't possible" LLM confusion ⇒ fallback to `event: COMMENT` (silent publish bypass); body populated with verdict summary; Conventional prefix missing on a lens-generated comment; verb "posted" used in the report instead of "drafted" |
...
- **Step 0's raw-arguments rule is non-negotiable.** The reviewer must never strip `--pr`, never paraphrase the user's literal arguments, never downgrade PR Mode to read-only because a parent prompt asked for "findings only". Reason: pending reviews are invisible until the user submits from the GitHub UI — the gate is in the human's hands, not the agent's.
