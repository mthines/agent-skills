...
| 1.7 | Review config load | [shared/rules/review-config.md](../../shared/rules/review-config.md) | `.review.yaml` hierarchy resolved; absent config → `profile: balanced` (defaults to profile: balanced — threshold 80, no placement cap, no filters) |
...
| 2.4 | Holistic review (default ON) | [shared/rules/holistic-review.md](../../shared/rules/holistic-review.md), `Skill("holistic-analysis", "review")` | Runs unless `--no-holistic` OR trivial-skip; emits 0–`max_findings` findings (3 / 6 / 10 by changed-file count) mapped to `issue` / `suggestion` / `nitpick` (reviewer asserts; cross-review questions) |
| 2.4c | Optimality review (default ON) | [shared/rules/optimality-review.md](../../shared/rules/optimality-review.md), `Skill("optimize-approach", ...)` | Runs unless `--no-optimize`, trivial-skip, or `incremental-quick`; 0–2 proposals rendered as cards in the report `Optimality` section, never as comment cards; `analysis_confidence` ≥ 85 is the only confidence gate; Fix / Self-Review may apply the top proposal at Step 4.1b |
| 2.5 | Dedupe + consolidate | [shared/rules/rubric-composition.md § Consolidation](../../shared/rules/rubric-composition.md) | Dedupe, group, priority-sort; **no cap of any kind** — confidence is the only gate; holistic claim wins on `(file, line)` collision |
...
| 2.4 | Default-on holistic call; trivial-skip heuristic (whitespace / dep-bumps / test-only / < 10 lines + no high-stakes); size-scaled `max_findings` budget (3 / 6 / 10); reviewer maps `system-fit` (major) → `issue`, `system-fit` (minor) → `suggestion` | Holistic skipped on a non-trivial diff that the heuristic incorrectly marked trivial; holistic finding overrides a line-level finding on the same `(file, line)` when the line-level was actually correct |
| 2.4c | Report-mode call; proposals bypass 2.7–2.9 and render as report cards; Step 4.1b apply outcome written back onto the card | Proposal reported as a comment card and bucketed by severity; apply outcome missing from the card and the counters; log block omitted at 0 proposals |
| 2.5 | Priority-sorted, uncapped; holistic claim wins on collision | LLM silently caps or summarises findings despite the rule; holistic-vs-line-level collision resolved wrongly |
