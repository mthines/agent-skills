...
- The L1 eval enumerates the lessons bucket names: Check E (`scripts/eval/l1.mjs`) asserts that no `memory/<bucket>/` directory is committed here, one assertion per bucket, so a rename must be mirrored into that array. That check keys on the bucket *name*, never on the `loop::…` tag.
- The **tag strings themselves are pinned** by L1 `G51`, which executes `scripts/eval/memory-loop-wiring.mjs` against replayed webhook fixtures: the runner resolves each record's tag from **this document** and follows it through the recorder and the workflow, so renaming a tag here — and nowhere else — turns L1 red (`G51c` and `G51d`, verified by doing exactly that). It pins the *chain*, not the storage: what a rename breaks is the addressing agreement between this enum, `scripts/record-comment-relevance.mjs`, and `.github/workflows/reviewer-comment-relevance.yml`, which is precisely the set a rename has to land in together.
