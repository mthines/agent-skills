...
The kind and host are **tracked as first-class LoreKit properties**, not inferred from the tag string on every read.
This document is the authoritative enum for their values.
...
Every `memory.write` from a loop sets `kind` and `host` explicitly.
The `loop::<host>-lessons` **tags stay** — they remain the cross-tool read filter and the back-compat contract — so this is additive, not a rename.

**Back-compat inference.** When a stored memory lacks `kind`/`host` (written before the property existed), LoreKit derives them from the tag so old memories gain the properties without a data migration:
...
**What the property unlocks** (not possible when the kind is buried in a tag string):

- Query by kind/host directly: `memory.list { kind: "lesson", host: "reviewer" }`, `lorekit ls --kind signal`.
- **Usage tracking** — memory operations are recorded per `kind` and per `host`, so reads, writes, and searches are attributable to a family and an owner instead of an opaque tag.
...
The clarity fix is the first-class `kind` + `host` **property** above plus **this document** — not a tag rename.
The property gives you kind-first querying (`--kind lesson`) and usage tracking without forking the tag contract or orphaning stored memories.
