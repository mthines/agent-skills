...
When the buttons are on (`agents/shared/rules/agent0-fix-links.md § Opt-in` — on by default; a repo opts out with `agent0_fix_links: false`), build the deep link and pass it as the payload's **`FIX_URL`**. `render-comment.mjs` renders the button itself, from `comment-spine.mjs`'s `fixButton()`, after the fix block — **never hand-write the markup**, since the host validation, the theme pair and the alt text all live in that one function and a second copy is how the report's button and this one drift apart. Build the link with **the same `--env <env>` — and the same `--org <slug>`, when the run resolved one — that `pr-reviewer.md § Fix-with-Agent0 buttons` resolved once for this run (`review-config.md § Run-level fields`); never rebuild or default the environment or the organization per finding — and `--source fix-this` (always this literal value here; `agent0-fix-links.md § Click attribution`).** `--org` is optional and absent by default (`agent0-fix-links.md § Organization`): a run whose repo set no `agent0_org` passes no `--org` argument at all, and that link is correct, not degraded. Omitting `--env` here silently falls back to `build-agent0-link.mjs`'s own `production` default regardless of `agent0_environment`, which is exactly the bug this line now guards against (a `development`-configured repo whose Fix-all report button correctly linked to `app.dash0-dev.com` while its inline Fix-this buttons still linked to `app.dash0.com`). `--source` has no default at all — the script throws rather than silently mis-tagging or dropping the click-attribution data.
