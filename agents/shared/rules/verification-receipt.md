...
`verify-behavior` is classified **spine** in that rule: a claim needing Tier 2/3 to decide is exactly the case the spine cap exists for.
Only when the file-presence check also fails does this rule degrade to today's static-only behavior — Tier 1 proof tools decide what they can, and a claim that needs Tier 2/3 to decide survives with only a partial (Tier 1) receipt, exactly as before this rule delegated — and that genuine skip is logged loudly via `RUN_ANOMALY` (never the silent `verify-behavior — not available, continuing` line alone) and caps the review tier at `standard`. Do not block the review on a missing skill.
