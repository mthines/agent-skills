...
Tier 3 itself now has a third approach alongside "run the covering test" and "synthesize a minimal repro": a claim about **cross-process behavior** (fan-out, retry count, span status on an error path, attribute cardinality) that neither a test nor a repro can observe directly delegates to `Skill("observe-run")`, which returns a receipt in this same four/five-token shape (see [`skills/quality/verify-behavior/rules/ladder.md`](../../../skills/quality/verify-behavior/rules/ladder.md#tier-3--execution)). This is still Tier 3 execution, still arrives at Step 2.6b, and still feeds Step 2.7 unchanged — `observe-run` is a third way to produce a Tier 3 receipt, not a new pipeline step. It skips silently when the skill or its Observability Profile dev target is unavailable, and the claim then falls back to the covering-test or synthesized-repro approaches above.

**When `verify-behavior` is unavailable:** resolution follows [`lens-invocation.md`](./lens-invocation.md) — try `~/.claude/skills/verify-behavior/SKILL.md` on disk (file-presence, never an error string) and follow it in-context before trusting any host resolution, since that path recovers the skill on hosts where `Skill()` itself cannot resolve the name.
...
Only when the file-presence check also fails does this rule degrade to today's static-only behavior — Tier 1 proof tools decide what they can, and a claim that needs Tier 2/3 to decide survives with only a partial (Tier 1) receipt, exactly as before this rule delegated — and that genuine skip is logged loudly via `RUN_ANOMALY` (never the silent `verify-behavior — not available, continuing` line alone), with `RUN.tier` omitted rather than overridden (`lens-invocation.md` § Enhancement vs. spine explains why a literal `standard` override does not survive `render-report.mjs`'s `full`-mode contract). Do not block the review on a missing skill.
