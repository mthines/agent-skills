...
| `F-warn-hides-open-threads` | Gate 3 rendering | Gate 3 graded ⚠️ and `UNRESOLVED_THREADS_SECTION` was omitted (or entries were dropped for being non-blocking), so softening the verdict also erased the worklist the author still needs | 4 |
...
| `F-report-accordion-flattened` | Report rendering | The report body was emitted without the `Review details` `<details>` wrapper — gate table, run mode, memories or quality lines at the top level — so the whole report renders expanded | 4a |
| `F-report-accordion-expanded` | Report rendering | The `Review details` block carried the `open` attribute, pre-expanding the diagnostics instead of letting the reader expand them | 4a |
| `F-open-threads-slot-orphaned` | Gate 3 rendering | Only one of the two Gate 3 slots rendered — the visible `UNRESOLVED_THREADS_SECTION` notice without `OPEN_THREADS_LIST` in the accordion (a pointer to nothing), or the list without the notice (a silent gate) — or the per-thread bullets were emitted at the top level | 4a |
...
- **Softening the verdict never shrinks the worklist.** `OPEN_THREADS_LIST` renders every open thread on ⚠️ exactly as on ❌; only the framing of the one-line `UNRESOLVED_THREADS_SECTION` notice changes. A thread may leave the list only by being resolved. The bullets live inside the `Review details` accordion and the notice stays visible above it — collapsing is never allowed to become omitting, and the visible notice is what guarantees that.
- **The report body is collapsed by default, and the wrapper is not optional.** Every diagnostic — gate table, run mode, memories, quality, integrations, optimality, standards, skipped files, and `OPEN_THREADS_LIST` — renders inside a `<details><summary>Review details…` block with no `open` attribute. Emitting those lines at the top level (`F-report-accordion-flattened`) or pre-expanding the block (`F-report-accordion-expanded`) is a guard failure even when every line is individually correct.
...
- Manual end-to-end: on a PR whose only open bot threads are a `nitpick:`/`suggestion:` with no blocking decoration and a thread declined on-thread with a rationale, confirm Gate 3 renders ⚠️ (not ❌), the verdict stays PASS/WARN, and `UNRESOLVED_THREADS_SECTION` still lists both threads under the `Open bot threads` heading. Under the old binary gate this PR fails until someone clicks Resolve.
