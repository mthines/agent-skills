...
| 1.2c | Diff-keyed lesson search (all modes) | [pr-reviewer.md § Step 1.2c](../../pr-reviewer.md) | One targeted `memory.search` keyed on changed paths, changed symbol names, and the synthesized intent + integrations; merged into the Step 1.0 pool |
