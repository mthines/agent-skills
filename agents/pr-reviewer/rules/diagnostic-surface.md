...
| `F-report-body-composed-from-memory` | Report rendering | `REPORT_BODY` was written from a recollection of the format instead of copied from its template — a reworded `<summary>` or heading, the pointer's ``Reviewed `<sha>` —`` opening on a full report, or an invented section (`**Verdict**`, `**CI**`, `**Prior findings**`) — so consumers matching literal headings silently miss it | 4a |
...
| `F-open-threads-slot-orphaned` | Gate 3 rendering | Only one of the two Gate 3 slots rendered — `OPEN_THREADS_SUFFIX` on the summary without `OPEN_THREADS_LIST` in the accordion (a count advertising nothing), or the list without the suffix (a silent gate) — or the per-thread bullets were emitted at the top level, or a retired standalone notice line was reintroduced between the headline and the accordion | 4a |
...
- **`REPORT_BODY` passes `report_body_is_safe()` before it is written.** The marker, the `Review details` accordion, no `open` attribute, nothing the accordion owns rendered above it, and no `**Verdict**` line. A failing body is re-rendered from the template, never posted as-is (`pr-reviewer.md § Render REPORT_BODY verbatim`).
