...
| 1.7b | Standards discovery (default ON) | [shared/rules/standards-conformance.md](../../shared/rules/standards-conformance.md) | Reuses `review-config.md` upward walk to discover governing docs (`CLAUDE.md`, `.claude/rules/*.md`, `AGENTS.md`, root slice); merges `.review.yaml` `standards:` entries; applies 30,000-char nearest-first cap; logs drops; caches as `STANDARDS_DOCS`; skips on `--no-standards`, trivial-skip, or `incremental-quick` (the same three conditions Step 2.4d skips on) |
