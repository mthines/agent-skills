...
Set `FIX_LINKS=on` when `--fix-links` is passed OR the resolved review config sets `agent0_fix_links: true` (default: **off** — emit no buttons and skip this block entirely). Also resolve the environment: read `agent0_environment` from the config (default `production`; `development` → `app.dash0-dev.com`) and pass it to the link builder as `--env <env>`. When on, render the "Fix with Agent0" buttons per `agents/shared/rules/agent0-fix-links.md`:
...
- **Fix this (inline).** When shaping an inline `issue:` / `suggestion:` finding (Step 2.8/2.9), append the Fix-this button as the final line after the fix block, per `comment-shape.md § Fix-with-Agent0 button`, built with the same script and the fix-this prompt template — filled with the finding's `path:line` and its own lead line as `{lead}` (drop any `"` from it). Skip `nitpick` / `question` / `praise`.
