...
- `polish` (`/`) — re-runnable pre-PR branch quality gate; thin orchestrator that composes the `reviewer` agent (auto-fix simple, plan complex) and `code-quality` simplify (apply Class M refactors). Modes: bare → full (review then simplify), `review`, `simplify`, `quick` (light mechanical pass). Commits each pass separately (`--no-commit` to skip). `/create-pr` delegates its pre-push step here (default → `quick`, `--review` → `review`, `--simplify` → `simplify`)
