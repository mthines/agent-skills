...
- `optimize-approach` (`Skill()`) — the fourth review lens: "is this the most optimal approach for its intent, and if not what is?" Judges four axes (codebase-fit, simplicity, performance, robustness) at the **approach level** with anti-overlap guards (defers line-level to `code-quality`, failure modes to `critical`, intent/system-fit to `holistic-review`) + a materiality bar; quiet early-exit stays silent when optimal. `report` mode (proposal) and `apply` mode (gated approach rewrite behind `confidence(code) ≥ 90 %` + scoped check + revert-on-failure; own-work only). Default-on lens in `reviewer`, `pr-reviewer` (report-only), and `polish` (apply) via [`agents/shared/rules/optimality-review.md`](./agents/shared/rules/optimality-review.md) at Step 2.4c; `--no-optimize` opts out. Never blocks the verdict. Self-improvement: `optimize-approach-lessons` fast tier (read O0 / write O5) calibrating the optimal/suboptimal bar + apply-safety; promotes to `diagnose`
