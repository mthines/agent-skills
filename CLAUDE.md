...
- `update-claude` — Update `CLAUDE.md`, `.claude/rules/`, **and `docs/`** based on code changes. Detects drift across all three tiers (dead `@imports`, stale narrative, hot-path leakage). Resolves placement automatically: a **Placement Resolver** (Phase 5c) routes each rule to the *innermost-ancestor* destination that still covers its scope — subtree-scoped rules land in `<dir>/CLAUDE.md`, cross-cutting patterns land in `.claude/rules/<topic>.md` with `paths:` globs (never root, since the point is to not pay tokens for them on every turn), narrative goes to `docs/`. Two targeted modes: `nested <dir>` (route all updates for a subtree to `<dir>/CLAUDE.md`, scaffold if missing) and `pattern <glob>` (discovery-driven — scan files matching `<glob>` for shared structure and emit a single path-scoped rule covering them)
