...
- `linear-planning` (`/`) — plan and create structured Linear projects / milestones / tickets from a rough idea **or a meeting transcript** (seeds scope from the [`meeting-notes`](./skills/analysis/meeting-notes/SKILL.md) skill instead of a cold interview). Scopes → searches for duplicate/related work → structures shippable milestones + vertical-slice tickets → previews the full structure as markdown → creates over MCP (`save_project` / `save_milestone` / `save_issue`) with dependencies wired. Four modes: `project`, `ticket`, `scope` (no writes), `audit`. Non-negotiable preview→create gate; never invents owners, dates, or estimates
...
- `meeting-notes` (`auto`) — distil a transcript (pasted / file / URL, any source) into scannable essence: TL;DR, decisions, action items (owner + due), open questions, key points, entities. Concise by design — never walls of text; degrades gracefully; never invents facts; emits stable headings so `linear-planning` and other skills can consume the output
