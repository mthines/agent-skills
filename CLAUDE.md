...
- `ux` — UX design review for web and React Native apps. Covers visual design, navigation, forms, touch and interaction, performance UX, platform-specific guidelines (Apple HIG, Material Design 3), WCAG 2.2 accessibility, UX writing / microcopy, and **dark-pattern / deceptive-design detection** (asymmetric consent UI, roach-motel cancellation, fake urgency/scarcity, drip pricing, confirmshaming, forced continuity, pre-checked opt-ins, manipulative AI surfaces) — grounded in Brignull (2010 / 2023), Gray et al. (2018), FTC 2022 *Bringing Dark Patterns to Light* + the 2024 Click-to-Cancel rule, EDPB Guidelines 03/2022, EU DSA Art. 25, EU AI Act Art. 5 + Art. 50, GDPR Art. 7, California CPRA, and OECD 2022. Hard rule: the skill **never** recommends or implements a dark pattern, even if explicitly asked — it flags as Critical and proposes the honest alternative
...
- `animations` — CSS-first web-animation slash command. Three entry points: (1) **Brainstorm Mode** — given an interaction ("press a button", "close a card", "delete a row", "open a modal"), runs a five-question framework (verb / reversibility / initiator / spatial source / affordance load) and looks up a catalog of ~40 interactions (discrete actions, element lifecycle, status & feedback states, continuous gestures, navigation) to recommend the right feedback shape, duration, easing, property, and intensity rung before any code is written — see [`rules/interaction-feedback.md`](./skills/animations/rules/interaction-feedback.md); (2) **Perceived-Performance Mode** — given an async wait (fetch, route load, image decode, optimistic mutation), consults a wait-duration ladder (< 100 ms → nothing; 100–300 ms → press-state only; 300 ms–1 s → skeleton or floored spinner; 1–3 s → skeleton + progress; 3–10 s → determinate progress; > 10 s → async-out) and ships the cheat-the-eye toolkit: skeleton loaders that match final content shape with shimmer in the 0.6–1.0 opacity band on `transform`-only, optimistic UI with mandatory visible rollback, the **200 ms loader floor + sub-200 ms skip** (no flash-and-glitch), progressive image loading (dominant colour / LQIP / blurhash / `loading="lazy"`), `font-display: swap;` with metrics-matched fallbacks, RSC streamed-content staggered reveals, predictive prefetch on hover / focus / viewport with debounce, and stale-while-revalidate with diff-targeted cross-fade — all grounded in Doherty (1982), Nielsen (1993), RAIL (Google), and Material progress conventions; see [`rules/perceived-performance.md`](./skills/animations/rules/perceived-performance.md); (3) the **technical workflow** — decision flow CSS → WAAPI → Motion → R3F. Covers GPU-safe properties (`transform`, `opacity`, `filter`), `will-change` discipline, CSS variable / `@property` interactive effects (cursor spotlight, magnetic buttons), modern entry / exit primitives (`@starting-style`, `transition-behavior: allow-discrete`, `interpolate-size`), View Transitions, scroll-driven timelines, state-choreography morphs (list ↔ stacked cards, full ↔ icon-only nav, grid ↔ detail) with a **mandatory pre-code planning checklist** and a dedicated accessibility section, React state patterns for driving them (`useMotionValue`, `AnimatePresence`, Server Components), advanced effects (Apple Liquid Glass, animated glow via pseudo-element + opacity, hover-to-expand, aurora gradient mesh, 3D pointer tilt), external engines (Lottie / dotLottie and Rive — when designer-authored playback or interactive assets beat code), React Three Fiber + Drei for 3D / WebGL scenes, and `prefers-reduced-motion` compliance throughout. Deliberately drops GSAP and `framer-motion` from its decision flow — both are superseded by **Motion** ([motion.dev](https://motion.dev), npm `motion`, import path `motion/react`, hybrid WAAPI + JS engine)
...
- `create-pr` — Generate a narrative PR description, push, then watch CI and auto-fix simple failures (lint, format, lockfiles); escalates judgment-required failures via `/confidence`. `--split` analyses the branch diff and breaks it into 2–4 dependency-ordered draft PRs (hard cap 5) after user approval, so reviewers don't have to digest a sprawling change in one sitting. `--review` (additive flag, default mode only) posts `@claude review` on the new PR, dispatches a `general-purpose` subagent with `run_in_background: true` that polls every 30s for up to 10 minutes for Claude's GitHub App to land a review (matches `claude` in `user.login`, submitted after the trigger timestamp), then invokes `/implement-suggestion` on the same PR — runs in parallel with the existing CI watch + auto-fix loop. Refuses `--split --review` at parse time. Skips silently with a one-line notice if the Claude GitHub App is not installed on the repo
- `create-skill` — Scaffold, review, upgrade, or diagnose agent skills (SKILL.md + rules/ + references/ + templates/) against best-practice frontmatter, progressive disclosure, token-aware structure, and the agent-skills.git symlink + inventory wiring. Modes: `scaffold` (default), `review`, `upgrade`, `diagnose`. **`diagnose <target-skill>` is the retrospective self-improvement entry point for any skill** in the repo: classifies a failed run against the target's declared diagnostic surface (phase model, failure taxonomy, existing-guards table, hard invariants — declared by each consumer in its own `rules/diagnostic-surface.md`), walks the phase-attribution matrix, runs `confidence(analysis) ≥ 90 %` as a hard gate, and emits an applyable unified diff against the target skill's source (`--apply` to apply locally, `--pr` to share upstream). Consumers today: `autonomous-workflow` and `fix-bug` (each declares its own `rules/diagnostic-surface.md`); the contract is reusable for `batch-linear-tickets` and any future skill that declares a surface
- `documentation` — Authors, audits, and maintains project documentation across every surface: agent-readable `CLAUDE.md` / `.claude/rules/` for the Claude Code hot path, cross-tool `AGENTS.md`, human-facing `README.md` (standard-readme spec, above-the-fold rule, badge hygiene), and `docs/` trees structured by the Diátaxis framework (tutorials / how-to / reference / explanation) — plus monorepo nesting and CI maintenance (Vale, markdownlint, alex, lychee, Conventional Commits → `release-please`). Four modes: `init` bootstraps a tiered docs setup from scratch by analyzing tech stack and complexity (small/medium/large tiers); `update` (default when `CLAUDE.md` already exists) diffs the branch against main, detects drift (dead `@imports`, removed files, renamed commands, stale narrative, hot-path leakage), and incrementally refreshes only what changed via a **Placement Resolver** that pushes rules to the innermost-ancestor destination (subtree rules to `<dir>/CLAUDE.md`, cross-cutting patterns to `.claude/rules/<topic>.md` with `paths:` globs — never root); `readme` scaffolds or audits a README against the above-the-fold checklist and the standard-readme spec; `audit` produces a comprehensive doc-health report across every surface (read-only). Two targeted update sub-modes: `nested <dir>` routes updates to a package-level CLAUDE.md, and `pattern <glob>` is discovery-driven (scans matching files for shared structure and emits one path-scoped rule). Replaces the previous separate `init-claude` and `update-claude` skills — same machinery, broader scope, single discovery surface. Routing canon ([`rules/content-routing.md`](./skills/documentation/rules/content-routing.md)) and Placement Resolver ([`rules/placement-resolver.md`](./skills/documentation/rules/placement-resolver.md)) are shared by every mode
...
- `fix-bug` — Single-bug counterpart to `batch-linear-tickets`. v2.1 ships an intake → **complexity triage** → evidence → preflight → reproduction-lock → analyse → gate → **lane-split handoff** → independent-verify → telemetry-verify pipeline (10 phases, one cross-cutting bug-notes ledger). Takes any starting evidence (Dash0 span / log / web event URL with UTC timezone compensation, raw stack trace, error message, code pointer `file:line`, Linear ticket URL via `linear-ticket-investigator`, screen recording via `/video-analyser`, free-text symptom). Phase 0 infers a `bugClass` (null-deref, race, off-by-one, contract-mismatch, perf, config, regression, logic). **Phase 0.5 runs complexity triage on a 14-row signal table** to pick `simple` (lightweight in-skill analysis + fast handoff lane) or `complex` (canonical holistic-analysis + standard lane). Conservative default is `complex` — when in doubt, run the slower lane. Phase 1.5 runs cheap pre-flight probes (recent commits, lockfile/env diff, last-known-green deploy SHA) which may upgrade triage `complex` → `simple` if the single-commit short-circuit fires. Phase 2.5 locks a failing reproduction by **delegating to `/tdd`** (unit/component/hook/integration), `/e2e-testing` (web flow), or `/e2e-testing-mobile` (Expo / React Native flow) — picks the lowest layer that captures the bug. Best-effort repros block the fast-lane. Phase 2c runs `git bisect run` if pre-flight identified a regression window. Phase 3 delegates root-cause to `holistic-analysis` (complex path) or runs a lightweight in-skill analysis (simple path); both emit a falsifiable root-cause paragraph. Phase 4 gates on `confidence(analysis)`. **At ≥ 92 % Phase 6 dispatches without human confirmation**: fast-lane (simple + ≥ 92 % + non-best-effort repro) routes `/fix-bug` → `Skill("aw-create-plan", ...)` → `aw-executor` and bypasses aw-planner; standard-lane (complex / downgrade / force-proceed) routes `aw-planner` → `aw-executor`. Both carry the CEGIS refinement contract (run repro → on failure capture counterexample → refine, cap 3 rounds); fast-lane round-3 failure falls back to standard-lane via aw-planner with the captured counterexamples — single-shot safety net for "triage classified simple but the bug wasn't." Phase 7 spawns `bug-fix-verifier` in fresh context to grade the PR (FAIL_TO_PASS, PASS_TO_PASS, diff sanity, repro integrity); identical for both lanes; only the verifier may undraft. Phase 8 (telemetry inputs only) polls the originating Dash0 query post-deploy filtered by release tag. Pass `--analyse-only` to stop after Phase 5 regardless of confidence (the primitive `/batch-linear-tickets` calls per ticket); `--force-holistic` skips the fast lane and always treats the bug as `complex`. Diagnosable via `/create-skill diagnose fix-bug` — phase model (now Phase 0–8 plus Phase 0.5 triage and Phase 6 lane split), F-novel-seeded failure taxonomy, existing-guards table, and hard invariants (only the verifier may undraft; verifier runs in fresh context; bug-notes ledger is append-only; three independent confidence gates **on both lanes**; fast-lane requires ≥ 92 % + non-best-effort repro; fast-lane round-3 fallback is single-shot; no force-proceed under 70 %; CEGIS capped at 3 rounds; telemetry bugs not done until Phase 8 closes the signal) declared in [`skills/fix-bug/rules/diagnostic-surface.md`](./skills/fix-bug/rules/diagnostic-surface.md). All sources at [`skills/fix-bug/references/research-sources.md`](./skills/fix-bug/references/research-sources.md)
- `implement-suggestion` — Implements reviewer suggestions across one or more PRs. Multi-PR mode (default when `$ARGUMENTS` contains PR URLs; empty `$ARGUMENTS` auto-detects the active PR for the current branch via `gh pr view`): per PR, resolves a `gw` worktree, fetches every actionable comment from **both human teammates AND AI code-review bots** (`claude[bot]`, `coderabbitai[bot]`, `sourcery-ai[bot]`, `sweep-ai[bot]` — login allowlist; noise bots `dependabot` / `github-actions` filtered) via `gh api /pulls/<n>/{reviews,comments}` + `/issues/<n>/comments` with a GraphQL resolved-thread filter, classifies each as `actionable` / `nit` / `discussion` / `question` / `praise`, validates each carry-forward comment through `/critical` (adversarial pre-mortem; severity-tagged findings) then `/confidence` (analysis mode; decision matrix `apply` / `surface` / `skip` with `Critical`/`High` `/critical` findings forcing `surface` regardless of score), and writes a plan.md-shaped suggestion-pack to `.agent/<branch>/suggestion-pack.md`. Phase 6 lane-split mirrors `/fix-bug`: **fast-lane** (single-file mechanical edits, ≤ 3 files, no `Major` `/critical` findings) dispatches a general-purpose worker subagent directly with the pack; **standard-lane** (cross-file edits, signature changes, `Major` findings, or ≥ 4 files) routes through `aw-planner` first to author `plan.md`, then dispatches the worker. The worker applies edits, runs scoped fast checks, commits one commit per PR with a narrative message, and pushes to the existing branch — **never opens a new PR**, never force-pushes, never skips hooks. Free-text mode (paste a comment / permalink) preserves v1 behavior: single validation pass, apply in cwd, no commit. Hard rules: workers have no retry budget (stuck = misclassification); `Critical`/`High` `/critical` findings are non-removable overrides; standard-lane below-gate stops (no silent fallback to fast-lane); resolved threads filtered at fetch time. Triggers on "implement suggestion", "apply review comments", "address PR feedback", "/implement-suggestion"
- `optimize-mock-data` — Optimizes a directory of structurally-related JSON / JSONL mock fixtures by inferring a shared schema, detecting structural drift between files, normalizing formatting and key order, and optionally shrinking verbose payloads while preserving shape. Three modes: `analyze` (default, read-only) emits a cluster + drift report with HIGH/MED/LOW severity; `normalize` rewrites files in place with 2-space / LF / trailing-newline canonical form (optionally `--fill-missing` to backfill keys as `null`); `shrink` truncates oversized string fields with a `…(truncated, was N chars)` marker (idempotent; refuses to touch `*Id`, `hash`, `actionId`, `threadId`, `userId`, `panelId`, `dashboardId`, `traceId`, `spanId`, `sessionId`, discriminator tags, URLs, JSON-in-string, base64, or PEM blocks). Ships four stdlib-only Python scripts ([`shape.py`](./skills/optimize-mock-data/scripts/shape.py), [`diff-shapes.py`](./skills/optimize-mock-data/scripts/diff-shapes.py), [`normalize.py`](./skills/optimize-mock-data/scripts/normalize.py), [`shrink.py`](./skills/optimize-mock-data/scripts/shrink.py)) that run via `Bash` — no `pip install`. Every rewrite runs a round-trip parse check and reverts if the result doesn't match the original Python object. Largest cluster is the reference; outliers get per-key-path drift sites sorted by frequency. Use when fixtures have grown inconsistent (mixed tabs / 2-space, reordered keys, fields in some files but not others, megabyte payloads), when adding a new mock that must match an existing set, or when reducing fixture token cost for a test suite. Diagnosable via `/create-skill diagnose optimize-mock-data` — phase model, F-novel-seeded failure taxonomy, existing-guards table, and hard invariants (round-trip parse safety non-removable; `analyze` mode never writes; protected-key list append-only; idempotency contract on `normalize` and `shrink`; minimum-2-file gate; reference cluster = largest cluster mechanically; shape extraction omits leaf values; stdlib-only Python; arrays stay ordered) declared in [`skills/optimize-mock-data/rules/diagnostic-surface.md`](./skills/optimize-mock-data/rules/diagnostic-surface.md)
- `persistent-memory` — Persists context across conversations as plain markdown so every future session can read prior conversations and enrich a specific scope (e.g. `parenting`, `relationship-anna`, `work-history`, `project-acme`). Slash-only **by design** — the description does not load into every session; the skill is invoked either explicitly (`/persistent-memory write parenting`) or at runtime via `Skill("persistent-memory", "read <scope>")` from a host skill's pointer block. Four operations: `write` (extract candidate memories from the conversation, resolve each against existing entries with ADD / UPDATE / DELETE / NOOP — pipeline modeled on **Mem0**), `read` (load a curated INDEX into context, fetch detail entries on demand — progressive disclosure modeled on Claude Code's `MEMORY.md`), `consolidate` (sleep-style compression that merges duplicates, summarizes stale entries, prunes obsolete facts), and `forget` (user-driven delete or redact with full audit trail). Storage tiered: `home` (`~/.agent-memory/<scope>/`, default, cross-project, never committed), `project-local` (`.agent/memory/<scope>/`, gitignored), `project-shared` (`memory/<scope>/`, committed). Per-scope directory layout: `INDEX.md` (≤ 200 lines, always loaded by `read`), `entries/<yyyy-mm-dd>-<slug>.md` (loaded on demand), `archive/` (audit retention), `AUDIT.log` (append-only NDJSON ledger). Strict never-store list (passwords, API keys, JWTs, credit cards, SSNs, private keys); PII like names/ages/addresses is allowed but flagged in the consent preview. Every `write` shows the user a candidate plan before persisting; `--auto` skips the prompt but still prints the plan. Memory taxonomy from CoALA (Princeton): `semantic`, `episodic`, `procedural`, `preference`; only `episodic` auto-archives (180 days, unreferenced). Documents four scaling tiers — plain markdown (default), markdown + SQLite FTS, markdown + local vector DB (Chroma / Qdrant / LanceDB), managed memory layer (Mem0 / Letta / Zep) — with migration recipes. Integrates with other skills via a pointer-based pattern: target skill adds a one-line `Skill("persistent-memory", "read <scope>")` block to its `SKILL.md` (default), or opt-in `SessionStart` hook for always-on scopes. Sources: Mem0 (arXiv 2504.19413), MemGPT / Letta, Memori (arXiv 2603.19935), Zep, SCM sleep-consolidation (arXiv 2604.20943), Reflexion entrenchment warning, Claude Code `MEMORY.md` docs, "Privacy by Design for Gen AI" three pillars (redaction / minimization / consent), OWASP LLM01:2025
...
- `reviewer` — Constructive code reviewer with auto-fix, report, and PR comment modes. Orthogonal `--with <skill1>,<skill2>` flag loads up to 3 additional review lenses from `skills/<name>/lens.md` (each capped at 80 lines / 600 tokens) and applies them as extra rubric criteria — see the [Review-Lens Contract](./skills/create-skill/rules/review-lens-contract.md) and the [`lens.md` template](./skills/create-skill/templates/lens.md). Opt-in lenses currently available: [`ai-engineering`](./skills/ai-engineering/lens.md), [`animations`](./skills/animations/lens.md), [`charting`](./skills/charting/lens.md), [`dx`](./skills/dx/lens.md), [`holistic-analysis`](./skills/holistic-analysis/lens.md), [`ux`](./skills/ux/lens.md) (canonical interface; reviewer auto-loads `ux` on UI diffs so `--with ux` dedupes to a no-op). Diagnosable via `/create-skill diagnose reviewer` — phase model, F-novel-seeded taxonomy, existing-guards table, and hard invariants (PR-review `body == ""`, `event` field omitted entirely to create PENDING, Conventional-Comments prefix on every comment, never `gh pr comment`, comment-confidence threshold ≥ 70 %, "drafted" not "posted" in user-facing report) declared in [`agents/reviewer/rules/diagnostic-surface.md`](./agents/reviewer/rules/diagnostic-surface.md)
