...
- `reviewer` — own-work code reviewer (own branch or own PR). Three sub-modes: Fix (auto-fix simple + plan complex), Report (`--report`, propose only), Self-Review (own PR, auto-fix + inline terminal report). Never writes to GitHub — redirects to `pr-reviewer` on a cross-author PR. Default-on **optimality lens** (Step 2.4c) via `optimize-approach` — applies the top approach rewrite in Fix / Self-Review (`--no-optimize` opts out), routed through `agents/shared/rules/optimality-review.md`. Imports shared rules under `agents/shared/rules/` including six shared rules: `verification-receipt.md` (Step 2.6b — executed proof for behavioral claims; null = DROP), `outcome-learning.md` (resolution-rate feedback loop; consumes the `review-outcomes` bus for promotion — never per-review), `review-outcomes.md` (shared candidate/outcome bus schema, fingerprint, TTL, consolidation — produced by `implement-suggestion`, consumed at promotion time only), `review-config.md` (`.review.yaml` profile / filters / path-instructions; hierarchical discovery; absent → `profile: balanced` = today's defaults), `prior-comment-awareness.md` (Self-Review: dedup against prior bot comments + anti-flip-flop on resolved suggestions), and `comment-relevance-memory.md` (**new** — per-repo LoreKit relevance signal: reads `reviewer-comment-relevance` at Step 0.7 to drop/downgrade known noise patterns and promote reliably-resolved ones before filter suppression; Step 2.2 in the pipeline)
- `pr-reviewer` — cross-review reviewer for someone else's PR. Authors short, grounded, confidence-gated inline comments and (with `--publish` or an explicit authorization phrase) posts them as a PENDING review invisible to the author until you submit from the GitHub UI. Refuses on your own PR (points to `reviewer`). Imports shared rules under `agents/shared/rules/`; owns auth gate + posting mechanics + line validity under `agents/pr-reviewer/rules/`. Two-tier holistic review: a broad whole-PR pass (Step 2.4) plus default-on **targeted escalation** (Step 2.4b) that fans out parallel, single-target holistic traces on context-dependent findings (changed exports whose correctness depends on caller context); cap 10, `--no-escalate` to skip. Default-on **optimality lens** (Step 2.4c) via `optimize-approach` in report-only mode (cross-review never rewrites; `--no-optimize` opts out), routed through `agents/shared/rules/optimality-review.md`. Also imports six shared rules: `verification-receipt.md` (2.6b), `outcome-learning.md` (post-merge via `/review-outcomes`; consumes `review-outcomes` bus at promotion time only — never per-review), `review-outcomes.md` (shared candidate/outcome bus — produced by `implement-suggestion`, consumed at promotion time only), `review-config.md` (1.7 config load), `prior-comment-awareness.md` (default-on prior-comment dedup + anti-flip-flop), and `comment-relevance-memory.md` (**new** — per-repo LoreKit relevance signal: reads `reviewer-comment-relevance` at Step 1.0 to drop/downgrade known noise patterns; Step 2.2 in the pipeline). Memory read is apply-aware: `reviewer-lessons` are matched by `trigger-context` at Step 1.0 and applied as pipeline considerations (not just loaded), and a diff-keyed `memory.search` (Step 1.2c) augments the top-50 `memory.list` on large repos. The write model is deliberately **outcome-driven** — `pr-reviewer` writes no lessons in-run (fresh-eyes freshness; five entrenchment guards documented in Step 1.0); all `reviewer-lessons` writes flow through `outcome-learning.md` post-merge and conform to the shared lesson-scope schema (`trigger-context` + `expires`). On a **re-review** (commit-triggered second+ pass) it also runs `thread-resolution.md` (Step 4.5): it auto-resolves its own prior GitHub threads that are now fixed or declined (`resolveReviewThread` GraphQL, never touching `persisting`/`unaddressed` or others' threads) and writes the fixed/declined outcome to the `reviewer-comment-relevance` Signal bucket (`trigger: re-review-reconcile`) — so a re-run cleans up after itself and the per-repo relevance signal accumulates on every commit, not just at merge
