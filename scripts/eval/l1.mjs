...
// contract is a small pure function, not a rendered template) plus the CLI's two fail-closed guards.
{
  ...
  // G32: the Agent0 fix prompts are one `/implement` invocation plus an address — the skill owns the
  ...
  // what /implement now owns — an embedded gh api graphql worklist, a {lead} excerpt, a {count}
  ...
  const implementTpls = templates.filter((t) => t.trimStart().startsWith("/implement"));
  ...
  s.check("G32a the Fix-all template is an /implement call carrying the PR URL and the reviewer's login",
    !!fixAll && /^\/implement https:\/\/github\.com\/\{owner\}\/\{repo\}\/pull\/\{n\} \{bot_login\}$/.test(fixAll.trim()),
    `no text-fenced template is a bare "/implement <pr-url> {bot_login}" — got ${JSON.stringify(fixAll ?? null)}`);
  ...
  s.check("G32c the Fix-this template is an /implement call scoping to one {path}:{line}",
    !!fixThis && fixThis.includes("/implement ") && fixThis.includes("{bot_login}"),
    "the fix-this template lost its /implement call, its {path}:{line} scope, or its {bot_login} author argument — without the scope it duplicates Fix-all, and without the author /implement skips a bot reviewer's own findings");
  ...
  s.check("G32l the Fix-all login fallback passes the report comment permalink to /implement",
    !!fallback && /^\/implement \{report_comment_url\}$/.test(fallback.trim())
  ...
  // Nothing /implement owns may be re-inlined into a prompt. Each of these three IS the shape the
  ...
  for (const [what, re, why] of [
    ["an embedded gh api graphql worklist", /gh api graphql/,
      "/implement gathers the PR's comments itself; an embedded query is a second implementation that drifts"],
    ...
    ["a {count} checksum", /\{count\}/,
      "/implement reports what it applied; a count in the URL is a second, staler answer to the same question"],
  ]) {
    s.check(`G32m no /implement template re-inlines ${what}`,
  ...
  // of a default nginx/Apache. G32k adds a much tighter bound to hold the /implement win: without it,
  ...
  for (const [name, tpl, source] of [["Fix-all", fixAll, "fix-all"], ["Fix-this", fixThis, "fix-this"]]) {
    ...
    s.check(`G32k a filled ${name} URL stays under the ${SHORT}-char /implement bound`,
      len < SHORT,
      `filled URL is ${len} chars — the /implement rewrite put both prompts near 200–300, so ${len} means a clause, a worklist, or an excerpt crept back in (agent0-fix-links.md § Deep-link format)`);
  }
  ...
  // method — the two /implement templates delegate verification to the skill — so it must route to
  ...
  s.check("G32n the CI-only template is not an /implement call",
    !!ciOnly && !ciOnly.includes("/implement") && /must not become one/.test(rule),
    "the CI-only variant became an /implement call — /implement applies review comments, and a red check with zero findings has none to apply");
