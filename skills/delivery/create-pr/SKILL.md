...
| 3 | `--no-simplify` (or legacy `--review` alone)             | `Agent(subagent_type: "pr-reviewer", prompt: "<pr-url>")` | `pr-reviewer` **agent** (Agent tool, not `Skill()`) one-shot only — findings surfaced, not applied. |
