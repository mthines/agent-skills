...
- `python3 skills/optimize-mock-data/scripts/shape.py <file> --sha256` — verify shape extraction still produces a deterministic hash on a known fixture (sha256 must match a pre-recorded value).
- `python3 skills/optimize-mock-data/scripts/diff-shapes.py <dir>` against a curated single-cluster fixture set — must exit 0 and report "None — every file shares the reference shape."
- `python3 skills/optimize-mock-data/scripts/normalize.py --in-place <file> && python3 skills/optimize-mock-data/scripts/normalize.py --in-place <file>` — second invocation must print "no changes needed" to stderr.
- `python3 skills/optimize-mock-data/scripts/shrink.py --in-place --max-string 200 <file> && python3 skills/optimize-mock-data/scripts/shrink.py --in-place --max-string 200 <file>` — second invocation must report `0 truncated` (idempotency).
...
- `python3 skills/optimize-mock-data/scripts/trim.py --in-place <file> && python3 skills/optimize-mock-data/scripts/trim.py --in-place <file>` — second invocation must print `no changes` to stderr (idempotency).
