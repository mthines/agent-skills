...
description: >
  ...
  change — verifying a View Transition, a Motion `layout` morph, a hover
  ...
  `pr-reviewer` cannot read from code, and by the `pr-reviewer` agent to attach
...
- A static screenshot is sufficient — capture a still directly (the
  `pr-reviewer` agent's visual pass owns PR-attached screenshots).
...
| `pr-reviewer` | In PR Mode when the diff matches `animations` / `ux` heuristics and the PR author has not attached a recording. | `url`, `selector`, PR number (for the upload path). |
