...
| `review`   |         | PR validation — does this diff implement what its description claims, and does the change make sense given how the changed code is used in the wider system? Returns structured findings for a reviewer agent to consume — **not** root-cause analysis. Skips Phases 2–8; see [Review Mode](#review-mode-pr-validation-flow). |

The mode determines how certain phases are framed (noted inline below). `fix` and `refactor` walk the full eight-phase protocol. `review` is a focused three-phase variant designed for the `reviewer` / `pr-reviewer` agents — see the section linked above.
...
`review` mode is a focused variant designed to be invoked from the `reviewer` and `pr-reviewer` agents as part of their per-PR pipeline. It validates **two orthogonal things** the line-level rubrics can't see:
...
The output is **structured findings** the calling agent consumes — not root-cause analysis, not a plan, not a confidence score. Phases 2–8 of the full protocol are skipped; only a streamlined three-phase flow runs.
...
Run both in order. Each emits 0–N raw findings; the caller will gate them through grounding + per-comment-confidence + shape downstream.
...
This is a recommendation; the calling agent makes the final mapping.
...
- **It does not block.** The verdict decision remains with the caller — the four strict blocking categories in `reviewer.md` and `pr-reviewer.md` (broken behaviour, security, data loss, **misimplemented intent**) gate "Request changes". Review mode supplies evidence for the fourth.

### Skipping

Review mode is opt-out (the calling agent enables it by default). Skip when the diff is genuinely trivial — the calling agent owns the skip heuristic; this skill executes if invoked.
