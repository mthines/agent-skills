...
1. **Every wait bounded at both levels** — inner `timeout` under the cap *and* the explicit `timeout: 600000` tool parameter, at all seven sites, enforced per-site by L1 check `G22` (proximity-scoped, count pinned; mutation-tested).
