...
2. **`plan.md` is the durable handoff artifact** in Full Mode — generated by
   the `aw-create-plan` companion, it carries the decisions, requirements, and
   acceptance criteria a fresh session needs to resume cold after compaction or
   a session boundary. Its evidence-backed payoff is cross-session resumption,
   not ceremony — so the template is tiered into an always-on **Core** and
   include-when-needed **Extended** sections, keeping small plans small. See
   ...
   §5 for the empirical basis (and the honest gaps).
