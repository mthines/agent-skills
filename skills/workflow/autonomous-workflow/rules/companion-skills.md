...
| `aw-review-quality-gate` | 6 | After `review-changes` returns findings — false-positive filter on the findings list (advisory) | —    | Remove invocation in [`phase-6-pr-creation.md`](./phase-6-pr-creation.md#findings-quality-gate) |
...
The `reviewer` agent's PR Mode behavior depends on authorship: for self-review (author == current user, detected via Rule 0 `gh api user` pre-check) it auto-fixes and emits an inline terminal report (Step 5.8); on a cross-author PR it **never writes to GitHub** — it refuses and redirects to the `pr-reviewer` agent, which owns pending-review authoring. Phase 7 PRs are always self-authored (aw-executor opens them), so the self-review sub-mode is the one this workflow exercises.
