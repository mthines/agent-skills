...
| `observability-coverage` | 3 | Files touched include a new/changed API handler, route, job entry point, or user-facing component/screen (see the skill's `scope-detection.md`) | `implement`           | Remove invocation in [`phase-3-implementation.md`](./phase-3-implementation.md#observability-trigger) |
...
| `observability-coverage` | 4 | Before Step 6 Final Validation — read-only gate on Phase 3's telemetry (`missing` findings on `web`/`mobile`/`api`/`worker` paths block; `unlinked` findings are advisory) | `audit --diff --base $(git merge-base HEAD main)` | Remove invocation in [`phase-4-testing.md`](./phase-4-testing.md#observability-gate) |
