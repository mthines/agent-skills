...
| 7     | CI watcher; `ci-auto-fix`; optional `reviewer` agent (PR Mode); `lorekit(memory.write loop::aw-lessons)` end-of-run + promotion check | CI passed because tests were narrow; `reviewer` not installed; recurring lesson (`seen_count >= 3`) not promoted to a permanent guard |
