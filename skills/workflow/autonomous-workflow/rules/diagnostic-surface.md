...
| 6     | `review-loop` (via `create-pr`); `aw-create-walkthrough`; `create-pr`                                                           | Reviewer didn't compare diff against `plan.md`; walkthrough hid the issue                                     |
...
| F5      | PR-delivery polish/review collapse | Phase 6 opens the draft PR without create-pr's default post-draft quality loop (`pr-reviewer` auto-fix + `code-quality simplify`) and/or the Phase 6 critical self-review actually running or running observably — often an over-correction that misapplies a repo's parallel/cascading-verify (lint/tsc/test execution) prohibition to the *allowed* sequential quality pass. Tell-tales: missing `walkthrough.md` in Full Mode; create-pr report carries the CI block but no polish summary and no reviewer-feedback block. | 6 | `rules/phase-6-pr-creation.md` — Phase 6 Delivery Receipt gate (bare-create-pr requirement + deterministic `walkthrough.md` check + per-sub-step receipt) |
| F6      | Capability gate written as a tool-name check | A dispatch-availability gate tests for the literal tool name `Task` rather than for the capability, so a harness that spells the tool `Agent` (the Claude Agent SDK harness behind Claude Code on the web and in cloud sessions) is read as having no sub-agent dispatch. Downstream, the run takes a degraded path it did not need — `review-loop` skips at iteration 0 and the PR ships `NOT REVIEWED`, or the Full tier drops to single-context. Self-concealing: every degraded path reports as a legitimate outcome, so nothing contradicts the misread. | 6 → 7 (and `aw` routing) | `review-loop` Step 0 precondition, `pr-review` Step 2, `aw`'s routing + review recovery — all three must test the capability (does any available tool dispatch a sub-agent?), never one spelling |
| F7      | Recovery path re-runs the failed dispatch | A recovery step exists for a capability the recovering context also lacks, and invokes the same thing that just failed instead of the degraded variant — so it consumes a step and recovers nothing. Instance: `aw`'s review recovery called `Skill("review-loop", …)` bare after the executor's review skipped, and skipped again at iteration 0 whenever `aw` itself held no dispatch tool. | 6 | `aw`'s "Review recovery when the executor could not dispatch" — branch on the recovering context's own capability, and fail closed toward the degraded variant (`--external-review`), never toward the skip |
...
- **A GitHub *access path* is required; `gh` is one of two, and `gw` is optional with a native fallback.** Phases 6–7 resolve `ACCESS_PATH` (`gh` | `mcp` | `none`) once via [`github-access.md`](../../../../agents/shared/rules/github-access.md) — never by `which gh`, never by `gh auth status`. A diagnoser must never propose reinstating a `which gh` hard-stop: `gh` is absent by design in Claude Code cloud sessions, where `mcp__github__*` is the path. With `none`, GitHub steps degrade and are named in `Degraded:`; they are never reported as done (L1 `G48`).
- **The system-prompt for the agent template stays lean.** It references `SKILL.md` rather than duplicating procedures.
- **The `aw` dispatcher is a skill, and the tier table has exactly one home.** `aw` is `aw/SKILL.md` (invoked `/aw`), not an agent: it runs in the caller's context so `aw-planner` / `aw-executor` are dispatched from the top-level rung and keep their own `Task` grant. It must not regrow a `tools:` block, a `model:` line, or an inline copy of the tier table — that table lives only in [`SKILL.md` Step 1](../SKILL.md#step-1-detect-workflow-mode-mandatory) and the dispatcher links it (L1 `G2b`). Because a skill cannot grant itself tools, a missing `memory.*` / `gh` / GitHub-MCP grant must be named in `Degraded:` rather than silently swallowed (L1 `G24b`). A diagnoser must never propose converting `aw` back to an agent to "give it tools" — see [`CLAUDE.md`](../CLAUDE.md#the-dispatcher-is-a-skill-not-an-agent--design-intent).
- **`aw` is the only natural-language entry point.** The dispatcher's `description:` owns the trigger vocabulary; the parent `autonomous-workflow` skill's description carries none of it and is reached by `/autonomous-workflow` or by `aw` itself. A request that lands on the parent directly runs the phases with tier detection and the universal lessons loop skipped — the two things the dispatcher exists to own. L1 `G2d` asserts both halves: the parent quotes no trigger phrase and makes no `Triggers on` / `Use when` claim, and every phrase `aw` advertises exists in [`routing.rule.md`](../templates/routing.rule.md) (which is also the live L2 `aw-should-trigger` rubric, so it can never be slimmed to remove the duplication). A diagnoser must never propose fixing a missed trigger by adding vocabulary to the parent's description — the fix belongs in `aw`'s description and the routing rule, in the same change.
- **Sub-agent dispatch availability is tested as a CAPABILITY, never as a tool name — and every degradation it feeds fails closed.** The dispatch tool is spelled `Task` in the Claude Code CLI and `Agent` in the Claude Agent SDK harness behind Claude Code on the web and in cloud sessions; other hosts may add further spellings. Each check — `aw`'s Full-tier routing and its review recovery, [`review-loop`](../../../quality/review-loop/SKILL.md)'s Step 0 precondition, [`pr-review`](../../../quality/pr-review/SKILL.md)'s Step 2 — asks whether *any* available tool dispatches a sub-agent (F6). Where the capability is absent, the branch taken is the degraded-but-productive one — `--external-review` for the review, single-context Full for the split. Where it is merely *undetermined* the two arms diverge deliberately, because their failure modes are asymmetric: the review arm pre-empts on undetermined as well (a skipped review is **silent** and unrecoverable, so `--external-review`'s worst case — one bounded wait that finds nothing — is strictly cheaper), while the split arm attempts the dispatch and falls back to single-context Full only when it fails **loudly** (`Failed to run agent`), because pre-emptively discarding context isolation costs more than one recoverable failed dispatch. Both arms fail closed under either state — never a silent skip (F7). A diagnoser must never propose reinstating a literal-name check, and must never propose a capability probe that *gates* a quality companion: that design was rejected in v3.22 because it fails open, and this check is admissible only because it gates nothing and fails closed. See [`CLAUDE.md`](../CLAUDE.md#adaptive-dispatch-the-aw-dispatcher--design-intent).
...
Diagnose Step 2 loads promotion-eligible lessons (the store's own `seen_count >= 3`, or the `status::structural` tag) as evidence — they are the strongest signal that a failure recurs. See [`self-improvement-loop.md`](./self-improvement-loop.md).
