...
| 7     | CI watcher (**bounded**: `timeout 540` per attempt, PR-scoped file-backed budget (`.agent/ci-watch-<pr>.state`) shared with `create-pr` Step 7, keyed on the **remote** PR head); `ci-auto-fix`; optional `pr-reviewer` (self mode, via `review-loop`); `lorekit(memory.write loop::aw-lessons)` end-of-run + promotion check | CI passed because tests were narrow; `pr-reviewer` not installed; recurring lesson (`seen_count >= 3`) not promoted to a permanent guard; **watch attempt exceeded the harness Bash ceiling so its expiry handler never ran (silent hang)**; **Phase 7 restarted a watch budget `create-pr` had already spent** |
...
- **Every wait on an external system is bounded, at both levels.** The enumerated set is: `gh ... --watch` / `gh run watch`, any poll loop that sleeps, and any command that blocks on CI, a deploy, or a remote queue. (`Task()` dispatch and `WebFetch` are **out of scope for this invariant** — a scoping decision, not a claim that they are bounded. A long-running sub-agent is a real way `aw` hangs; that duration is the dispatching agent's concern, tracked separately.) Two levels must both hold, and checking only the first is the trap: (a) the **in-command** bound — a `--watch` with no `timeout`, or a `timeout` larger than the harness cap, never fires its own expiry; (b) the **per-call tool timeout** — Claude Code's Bash tool defaults to 120 000 ms and maxes at 600 000 ms, so a correct in-command `timeout 540` issued at the default is still killed at 2 minutes and the expiry handling is still dead code. A grep that only checks (a) will pass a run that still hangs. Re-watching is allowed only against an explicit, PR-scoped, **file-backed** attempt counter (`.agent/ci-watch-<pr>.state`) — a shell variable cannot carry it, because shell state does not persist between Bash calls. A diagnoser must never propose removing the counter, moving it back into shell state, raising a per-attempt timeout above the harness cap, or omitting the explicit tool-timeout argument.
- **A run reports what it actually did, including what it skipped.** The universe of things that can be skipped is the registry in [`companion-skills.md`](./companion-skills.md) plus the agent companions listed there — "every companion" means every entry in that registry, not an open-ended set. `aw`'s terminal block carries a non-empty `Degraded:` line naming each one that did not run and why; the Progress Log carries the same in **Full tier**, where it exists (Lite and Micro have no `plan.md`, so the terminal block is the sole carrier there). A diagnoser must never propose suppressing a skip line to make a run read as clean — an unreviewed PR reported as reviewed is worse than a reported deviation.
