...
- **Every wait on an external system is bounded, and each attempt fits under the harness Bash ceiling.** A `--watch` with no `timeout`, or a `timeout` larger than the harness cap (600 s in Claude Code), is killed before its own expiry handler runs — the agent sees an opaque timeout with no instruction and hangs. Re-watching is allowed only against an explicit, PR-scoped attempt counter; an uncounted "retry the wait" is an unbounded loop. A diagnoser must never propose removing the counter or raising a per-attempt timeout above the harness ceiling.
- **A run reports what it actually did, including what it skipped.** `aw`'s terminal block and the phase Progress Log must name every companion or agent that did not run and why. A diagnoser must never propose suppressing a skip line to make a run read as clean — an unreviewed PR reported as reviewed is worse than a reported deviation.
