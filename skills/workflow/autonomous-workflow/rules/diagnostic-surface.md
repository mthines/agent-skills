...
- **Sub-agent dispatch availability is tested as a CAPABILITY, never as a tool name — and every degradation it feeds fails closed.** The dispatch tool is spelled `Task` in the Claude Code CLI and `Agent` in the Claude Agent SDK harness behind Claude Code on the web and in cloud sessions; other hosts may add further spellings. Each check — `aw`'s Full-tier routing and its review recovery, [`review-loop`](../../../quality/review-loop/SKILL.md)'s Step 0 precondition, [`pr-review`](../../../quality/pr-review/SKILL.md)'s Step 2 — asks whether *any* available tool dispatches a sub-agent (F6). Where the capability is absent or undetermined, the branch taken is the degraded-but-productive one — `--external-review` for the review, single-context Full for the split — never a silent skip (F7). A diagnoser must never propose reinstating a literal-name check, and must never propose a capability probe that *gates* a quality companion: that design was rejected in v3.22 because it fails open, and this check is admissible only because it gates nothing and fails closed. See [`CLAUDE.md`](../CLAUDE.md#adaptive-dispatch-the-aw-dispatcher--design-intent).
