...
| `observe-run` skill | Phase 2.5 repro-fidelity check — compares the local repro's trace shape against the originating production span, for a telemetry-sourced input only | Optional — advisory, silent no-op if unavailable or input is not telemetry-sourced |
