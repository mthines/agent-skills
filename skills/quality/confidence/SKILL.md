...
| 5 | Every requirement is tagged `[user-stated]` or `[inferred]`| `test "$(awk '/^## Requirements/{f=1;next} /^###? /{f=0} f' plan.md \| grep -c '^[-0-9]')" -eq "$(( $(awk '/^## Requirements/{f=1;next} /^###? /{f=0} f' plan.md \| grep -c '\[user-stated\]') + $(awk '/^## Requirements/{f=1;next} /^###? /{f=0} f' plan.md \| grep -c '\[inferred\]') ))"` exits 0 — the tag count must equal the requirement count (the range stops at `### Out of Scope`, so excluded items are not counted) |
| 6 | Every decision row has a Rationale column populated       | `awk '/^## Decisions/{f=1;next} /^###? /{f=0} f' plan.md \| grep '^\|' \| tail -n +3 \| awk -F'\|' 'NF<5 \|\| $4 ~ /^[ \t]*$/ {bad=1} END{exit bad}'` exits 0 — no missing or empty cells in the Rationale (third) column of the Decisions table |
| 7 | All timestamps are ISO 8601 with time component           | `test "$(awk '/^## Progress Log/{f=1;next} /^###? /{f=0} f' plan.md \| grep -c '^- ')" -eq "$(awk '/^## Progress Log/{f=1;next} /^###? /{f=0} f' plan.md \| grep -cE '^- \[20[0-9]{2}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z\]')"` exits 0 — every Progress Log entry starts with a `[YYYY-MM-DDTHH:MM:SSZ]` timestamp |
