...
Apply mode runs **only in own-work contexts** — a standalone `/optimize-approach apply` (`caller: user`). No agent or orchestrator invokes it; the `polish optimize` pass that used to was removed with `polish`.
...
Apply mode does not commit. The user owns committing — this keeps the rewrite revertible as its own reviewable unit.
