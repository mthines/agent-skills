...
| `documentation`      | 5     | Always (self-improving doc loop — keeps `CLAUDE.md`, `README.md`, `docs/` aligned) | `update`             | Remove invocation in [`phase-5-documentation.md`](./phase-5-documentation.md#documentation-trigger). Skip per-task by user override or skip-condition match (see [`phase-5-documentation.md#when-to-skip`](./phase-5-documentation.md#when-to-skip)). |
...
The `reviewer` agent's PR Mode behavior depends on authorship: for cross-review (author ≠ current user) it posts a **pending** GitHub review invisible to the PR author until submitted from the GitHub UI; for self-review (author == current user, detected via Rule 0 `gh api user` pre-check), it auto-fixes and emits an inline terminal report (Step 5.8) — no pending comments are posted (cross-review only; self-authored PRs receive inline terminal report instead).
