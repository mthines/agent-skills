...
aw-executor) by default. This single-agent template is no longer
...
- **`plan.md` is the single source of truth** in Full Mode — generated by `Skill("aw-create-plan")` inside the worktree, not on main.
...
- `aw-create-plan` / `aw-create-walkthrough` — artifact generators (Full Mode).
