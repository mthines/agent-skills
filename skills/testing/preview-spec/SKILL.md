...
| --- | --- | --- |
| `author` | first token `author`, or delegated from `create-pr` | Generate the spec for the diff and inject the marked collapsed block into the PR body. Reads memory first. |
...
2. **Read the diff.** Identify the changed screens, components, and user-visible behavior. `git diff <base>...HEAD --name-status` plus the relevant file contents.
3. **Write the spec** in `aw-tester`'s grammar, one `## Spec N:` block per user-visible behavior the diff changes. Prefer role-and-name locators; use `{testid: …}` only as an escape hatch. Keep it to the behaviors a reviewer would actually click through — 1 to 3 specs, not an exhaustive suite.
...
1. **Extract** the spec from the PR body between the `<!-- preview-spec:v1 -->` markers. Absent → report `no spec` and stop.
