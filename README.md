...
| **[persistent-memory](./skills/persistent-memory/SKILL.md)** | Persists context across conversations as plain markdown so every future session can read prior conversations and enrich a specific scope (e.g. `parenting`, `relationship-anna`, `work-history`, `project-acme`). Four operations: `write` (Mem0-style extract → ADD / UPDATE / DELETE / NOOP), `read` (progressive disclosure from an INDEX modeled on Claude Code's `MEMORY.md`), `consolidate` (sleep-style compression / pruning), `forget` (delete or redact with audit). Storage tiered: `home` (`~/.agent-memory/<scope>/`, default), `project-local` (gitignored), `project-shared` (committed). Strict never-store list (passwords, API keys, JWTs, credit cards, SSNs, private keys); PII like names / ages / addresses is allowed but flagged in the consent preview. Documents four scaling tiers — markdown → markdown + SQLite FTS → markdown + local vector DB → managed memory layer (Mem0 / Letta / Zep) — with migration recipes. Integrates with other skills via a pointer-based pattern (one-line `Skill("persistent-memory", "read <scope>")` block in the host skill's `SKILL.md`). | You want a topic-scoped memory ("parenting", "relationship-anna", "work") that a future conversation can read; you want the host skill (e.g. parenting advice) to remember prior interactions; you want to consolidate, prune, or forget specific entries; or you are deciding whether to stay on markdown or graduate to a managed memory layer. |
