...
| **[optimize-approach](./skills/quality/optimize-approach/SKILL.md)** | The fourth review lens — "is this the most optimal approach, and if not what is?" Judges codebase-fit / simplicity / performance / robustness at the approach level (anti-overlap guards defer to `code-quality`, `critical`, `holistic-review`); quiet early-exit when optimal. `report`, gated `apply`, and `plan` (plan-time) modes. Default-on in `pr-reviewer` (report-only — never applies in any relation), `polish`, and `aw-planner` Phase 1 (`plan`). Self-improving via `optimize-approach-lessons`. | `Skill()` |
