...
| **[review-loop](./skills/quality/review-loop/SKILL.md)** | Bounded convergence loop for one PR: `pr-reviewer` → `implement-suggestion --resolve-all` → `polish simplify` → CI, up to 5 iterations. Converges when every review thread is resolved (fix **or** honest reply) **and** CI is not red — a live finding it can neither fix nor honestly decline stays open (no green-wash). Sub-step D reads check state statelessly and delegates a red check to `ci-auto-fix` (cap 2 handoffs); `--no-ci` opts out and is passed by `create-pr` and `autonomous-workflow` Phase 7, which own the per-PR CI budget. `--external-review` waits on the shared [review-activity poll](./agents/shared/rules/review-activity-poll.md) for an out-of-process reviewer instead of dispatching `pr-reviewer` — the path for automation-triggered reviews, and for harnesses where `Task` is disabled. | `Skill()` |
