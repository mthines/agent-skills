...
| **[observability-coverage](./skills/quality/observability-coverage/SKILL.md)** | Ensures every delivery is measurable: RUM/analytics events for user-facing web/mobile changes (delegates to `rum-tracking`), OTel traces/metrics/structured logs for API/backend changes, and a named regression detector for every signal so errors and warnings never go silent. Modes: `guide` (default), `implement`, `audit` (advisory by default; `--strict` blocks on `missing` findings), `setup` (first-time interview recording the project's telemetry stack and, for monorepos, a per-package instrumentation map as a committed Observability Profile via `persistent-memory`). Wired into `autonomous-workflow` Phase 3 (author) and Phase 4 (advisory audit; `--observability-strict` to block). | `auto` |
