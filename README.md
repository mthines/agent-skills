...
|-------|--------------|------|
| **[autonomous-workflow](./skills/workflow/autonomous-workflow/SKILL.md)** | Phase-based orchestrator (0–7): task → plan → worktree → code → test → docs → draft PR → CI gate. Opt-in `aw` dispatcher routes by tier (Micro/Lite single-pass, Full → planner/executor split). Plan-quality gates (v3.15): missing-information gate, Existing Code Survey (anti-reinvention), requirement→criterion traceability, and an executable `checks.yaml` Phase 4 gates on mechanically. Universal two-tier self-improvement: episodic `aw-lessons` promotes to gated `diagnose` at `seen_count ≥ 3`. Repo-convention layer: a parallel `aw-conventions` loop — committed path-scoped `.claude/rules` (seeded by `aw-setup` repo-profile, authored via `docs`) plus gitignored learned deltas that promote up at `seen_count ≥ 3`. See [featured section](#featured-autonomous-workflow). | `auto` |
