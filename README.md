...
| **[optimize-approach](./skills/quality/optimize-approach/SKILL.md)** | The fourth review lens — "is this the most optimal approach, and if not what is?" Judges codebase-fit / simplicity / performance / robustness at the approach level (anti-overlap guards defer to `code-quality`, `critical`, `holistic-review`); quiet early-exit when optimal. `report` and gated `apply` modes. Default-on in `reviewer`, `pr-reviewer` (report-only), and `polish`. Self-improving via `optimize-approach-lessons`. | `Skill()` |
