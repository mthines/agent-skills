...
| Skill | What it does | Type |
|-------|--------------|------|
| **[code-quality](./skills/quality/code-quality/SKILL.md)** | Authors and reviews code for low cognitive complexity, guard clauses, early returns, single-responsibility. Four modes: `plan`, authoring (default), `review` (proposes), `simplify` (review-then-apply mechanical refactors behind `confidence(code) ≥ 90 %`; `deep` tier also auto-writes Class J dedup/structural/type-driven refactors behind a test-backed gate). | `auto` |
...
| **[polish](./skills/quality/polish/SKILL.md)** | Re-runnable pre-PR branch quality gate. Thin orchestrator over the `reviewer` agent (auto-fix simple, plan complex) and `code-quality` simplify. **Default is the `deep` tier**: bare `/polish` reviews, then auto-writes the bigger Class J refactors (dedup, structural, type-driven) behind a test-backed gate (unprovable ⇒ stays a proposal). Lighter escape hatches: `simplify` (Class M mechanical only), `quick` (light pass), `review` (reviewer only). Commits each pass separately. `/create-pr` delegates its pre-push step here — deep by default. | `/` |
