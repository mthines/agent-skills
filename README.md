...
| **[pr-reviewer](./agents/pr-reviewer.md)** | Unified PR review agent — handles both own-work review (self relation) and cross-review (cross relation) via a `REVIEW_RELATION` flag set at Step 0.5. Own-PR (self) mode: terminal report with assertive framing, auto-fix Simple findings, inline Self-Review report. Cross-PR (cross) mode: short, grounded, confidence-gated inline comments (≤ 240 chars, ≤ 2 sentences, `Skill("confidence")` ≥ profile threshold, default 80) posted as a PENDING review with `--publish`. Two-tier holistic review: a broad whole-PR pass plus default-on **targeted escalation** (Step 2.4b) that fans out parallel single-target holistic traces on context-dependent findings (cap 10, `--no-escalate` to skip). Shared rules: `verification-receipt.md` (2.6b), `outcome-learning.md` (promotion loop; never per-review), `review-outcomes.md` (shared bus), `review-config.md` (`.review.yaml` profile / filters), `prior-comment-awareness.md` (dedup + anti-flip-flop when PR exists). |
