...
| Skill | What it does | Type |
|-------|--------------|------|
| **[/create-pr](./skills/delivery/create-pr/SKILL.md)** | Narrative PR description, push, open PR, watch CI, auto-fix simple failures. Pre-push quality delegated to `polish`, **full review + simplify by default** (post-draft `review-loop`, invoked with `--no-ci` because Steps 7–9 own the CI watch); scale down with `--no-review`, `--no-simplify`, `--quick`, or `--no-quality`. Post-push reviewer-feedback loop default-on (`/implement-suggestion --watch`; `--no-feedback` skips). Step 10's report carries a **mandatory** `Preview spec` line on every run, non-UI diffs included — Step 6.4 has four skip conditions and one failure mode, and with no slot for them all six outcomes rendered as a clean, successful PR; Step 6.4 now records which of the six applied as it leaves, rather than leaving the slot to be reconstructed from memory. Step 5.5 converges the branch **before the push** via `review-branch`, which needs no PR — **opt-in with `--pre-review`** in default mode (Step 6.5 already reviews the same diff with the same detection core, so default-on would bill every PR twice for a trade nobody has measured), **default-on under `--split`**, where nothing reviews the whole branch otherwise: that slot previously ran `polish simplify` alone — mechanical refactors and no review — which is now only the fallback for `--no-pre-review` or an absent-dispatch skip. That skip reports as `NOT REVIEWED`, never as a skip, and the report's `Pre-push review` line is mandatory on every run including the one where the step does not fire. Flags: `--split`, `--quick`, `--pre-review`, `--no-pre-review`, `--no-review`, `--no-simplify`, `--no-quality`, `--no-feedback`, `--no-preview-spec`. | `/` |
