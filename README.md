...
| **[linear-planning](./skills/workflow/linear-planning/SKILL.md)** | Plan and create well-structured Linear projects, milestones, and tickets from a rough idea **or a meeting transcript** (seeds scope from the [`meeting-notes`](./skills/analysis/meeting-notes/SKILL.md) skill instead of a cold interview). Scopes → searches for duplicates/related → structures shippable milestones + vertical-slice tickets → previews as markdown → creates over MCP with dependencies wired. Four modes: `project`, `ticket`, `scope` (no writes), `audit`. Non-negotiable preview→create gate; never invents owners, dates, or estimates. | `/` |
...
| **[meeting-notes](./skills/analysis/meeting-notes/SKILL.md)** | Distils a meeting transcript (pasted text, file, or URL — any transcription tool) into scannable essence: TL;DR, decisions, action items (owner + due), open questions, key points, entities. Concise by design, degrades gracefully on messy input, never invents facts, and emits stable headings for downstream skills like `linear-planning`. | `auto` |
