...
| **[/preview-spec](./skills/testing/preview-spec/SKILL.md)** | Makes a UI PR autonomously verifiable. `author` injects a collapsed, machine-findable UI verification spec into the PR body (delegated to by `create-pr` on a UI diff; `--no-preview-spec` opts out); `run` resolves the PR's live preview deployment URL via the GitHub deployments API and runs the spec against it, reporting a pass/fail verdict. `--driver auto` (default) uses the fast in-session `aw-tester-chrome` runner when the Chrome extension is connected and falls back to the `aw-tester` Playwright sub-agent otherwise (CI / remote). Reuses `aw-tester`'s spec grammar and runner verbatim; owns the PR-embedding, the URL resolution, and a two-way lessons loop (the runner writes navigation friction to `preview-spec-lessons`; the author reads it plus `aw-tester-lessons` so future specs are correct from the start). Web only. | `Skill()` + `/` |
